...
class SafeDistanceQuantizedLabelFunction(SafeDistanceLabelFunction, BaseQuantizedLabelFunction):      
    ...
    def __init__(self, label_str: str, to_rear: bool, delta_ego: float, delta_others: float, a_e: float, a_o: float, 
        ...
        self.InitializeSpecs(signal_sampling_period)     

        self.signal_sampling_period = signal_sampling_period
    ...
    def GetSignalSamplingPeriod(self):
        return self.signal_sampling_period

    def GetRobustnessNormalized(self):
        return self.robustness_normalized

    def InitializeSpecs(self, signal_sampling_period: float):
    ...
    def ComputeRobustness(self, eval_result):        
        safe_distance = eval_result 

        if (self.stl_spec_lon_checked):
            self.robustness_lon = self.NormalizeRobustness(self.robustness_lon, self.feature_range, self.lon_value_range) 

        if (self.stl_spec_lat_checked):
            self.robustness_lat = self.NormalizeRobustness(self.robustness_lat, self.feature_range, self.lat_value_range) 
    ...
    def Evaluate(self, observed_world):        
        ...
        self.ComputeRobustness(next(iter(eval_result.values())))
